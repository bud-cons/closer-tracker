...
// resulting percentage is then applied to that same monthly Cash Collected
// total, and that dollar amount is added on top of Commission.
...
export function bonusAmountForCashCollected(cashCollected: number, bonusPercent: number): number {
  return cashCollected * bonusPercent;
}
